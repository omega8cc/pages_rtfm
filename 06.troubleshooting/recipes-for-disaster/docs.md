...
"good habits":good-habits, you can get stuck with a broken site that @Restore@ won't fix.
...
platform*, and then @Migrate@ the _clone_ to the new platform. If one
...
and @Migrate@ your live site directly to the new platform _without_
...
your _database_, but leaving the site on the new platform. All your
old backups can no longer be used by the @Restore@ task.

You can still use these backups to "re-create the site manually":aegir-import
from scratch, as you typically do when you import the site into Aegir.

h2. Always "Clone" to the Current Platform
...
h2. Always Create Platforms in "~/static"
...
h2. Always Install AdvAgg To the Right Directory
...
However, because Aegir already has such granular "caching mechanisms":aegir-caching
...
h3. Enable All the AdvAgg Submodules You Need
...
h2. Always Put Contrib Modules in @sites/all/modules/@
...
Each site doesn't have to _enable_ all these modules. But no site
...
But these private copies will eventually cause *broken migrations*, as
you try to @Migrate@ your site to a new platform with upgraded
modules. Let's see why.

h3. You Forget Which Modules You've Stashed
...
platform, you _really_ don't want every upgrade to involve checking
...
h3. Aegir Only Compares @sites/all/modules/@

There's an even more serious reason to keep all your modules in
@sites/all/modules/@. Those are the modules that *Aegir manages*. When
...
know_. It will _upgrade the database_, thinking the site needs to work
...
h3. What If I Need a Different Version of the Module?
...
collection of modules _of particular versions_ that all work together.

Note that if you just want to _add_ a new module, you don't have to
...
h3. Custom Modules Can Go in @sites/foo.com/modules/@
...
/$yourprofile/(modules|themes)@ for a site using that install profile....
...
p. If you want to understand more, we recommend you "read the entire
