...
h2. Quickstart Guide to Disabling Caching
...
# Add ?nocache=1 to the URL and reload.

# Add ?noredis=1 to the URL and reload.

# Edit a node or block on the page. This can force the cache to
  reload.

# If the page is a view, check whether you need to disable caching for
  the view.

# Purge all Boost's caches and then "disable the module"#disable-boost.
  The order is very important here.

# "Use a .dev. or .devel. alias":#caching-dev for the site, visit the
...
# Use the "AdvAgg module":#use-advagg, or, if you're already using it,
...
h2. Overview: Disable Caching So You Can Change Settings
...
h3. Drupal 6 Performance Page Settings
...
Javascript Files* are Forced _Disabled_. You must "configure AdvAgg":module-advagg
instead.

h3. Drupal 7 Performance Page Settings

Access the Drupal 7 Performance page at: @admin/config/development/performance@).
...
h4. Overview: The ".dev." or ".devel." Alias
...
h4. Step-by-Step: Add a ".dev." or ".devel." Alias

# In Aegir, click "Sites".

# Click your site name. This brings to your site node.

# Click "edit".

# Add an alias that includes @.dev.@ or @.devel.@
...
# Save the node and wait until the Verify task completes.
  You can now access your site via this alias! Visit the Performance
  page via this alias to change the desired settings.

The public domain name for this site will _not_ be affected. If you
...
Using @.dev.@ or @.devel.@ will _allow_ to disable caching, but it does
...
h3. Which Caching is Enabled Automatically?
...
h3. Which Caching is Supported But Not Enabled Automatically?

Our hosting also includes caching/aggregation which is _supported_,
...
* "AdvAgg module":module-advagg, which is supported but must be downloaded.
