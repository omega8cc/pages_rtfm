...
h2. Task: Delete a Site/Platform Node Manually

You want to delete a site or platform, but the Aegir @Delete@ task
fails. Now what?

# A word of warning: never, ever attempt to delete the site or platform
  files and directories manually via SFTP, FTPS or "SSH":ssh -- you will
  only make things (much) worse to clean up. You should always allow
  Aegir to delete the site (or platform) by running the @Delete@ task,
  even if it will eventually fail.

# First, @View@ the details (a log) of why the @Delete@ task failed.
  Try to debug the failure. Once you delete the site or platform node
  in the Aegir control panel, you'll also delete all associated task nodes,
  and you'll lose the option to restore from any existing Aegir backups
  of the site. So be sure you can accept these side effects before you proceed.

# Click the @Edit@ tab for the site or platform.

# In the URL for this edit page, replace @edit@ with @delete@. If the
  path ends in @node/123/edit@, change it to @node/123/delete@.

# Press @ENTER@ to visit the new URL you've typed.

# On this delete page, click @Delete@, and confirm.

This should delete the site's or platform's node in Aegir (not the site
or platform on the filesystem, obviously).

Note that the '@View@ task log' hint is useful only for already failed and
not still running ("spinning forever") tasks, because when the task is still
in the pseudo-running state, its log will be mostly empty or incomplete.

Remember that you have just deleted only a record in the Aegir control panel,
...
If the
leftovers on the system are incompletely removed after the failed @Delete@ task,
you should "contact support":support for assistance.

h2. Task: Delete a Task Node That's "Spinning Forever"

You can stop any task by manually deleting its node.

Note: @Clone@ or @Migrate@ tasks will require extra attention (see below).

# Make sure you wait at least *15-30 minutes* before manually deleting a task node.
  There are heavy distros like the new Open Atrium 2.0 where even the initial
  site install may take longer than 10 minutes to complete.

# Right-click on the @View@ link of the “spinning” task and select
  @Open in New Window@ or @New Tab@. This brings you to the node for this task.

# Click on the @Edit@ tab of the task node.

# In the URL, replace @edit@ with @delete@. If the URL is
  @node/123/edit@, change it to @node/123/delete@.

# Press @ENTER@ to visit the new URL you've typed.

# On this delete page, confirm the deletion.

# If the task was running on a site, @Verify@ the platform where the
  site is (or should be). Aegir will re-discover and @Verify@ the
  site, which will generate a new node for the site.

h2. Task: Fix a Broken "Clone" or "Migrate" Task
...
# Make sure the site was actually migrated or cloned. Check the
  filesystem via SFTP or SSH and see whether the new directories are
  present on the correct platform.

# Delete the *site node* for the migrated or cloned site. Do *not* use
  the @Delete@ task! Instead, follow the steps given above for
  deleting a site node manually. (See below for why you must delete
  this site node.)

# Delete the *task node* for the spinning @Clone@ or @Migrate@
  task. Follow the steps given above for deleting a task node.

# When you verify the platform, Aegir should discover your new cloned
  or migrated site.

h2. Task: Fix the Huge "files/" Problem
...
*WARNING:* You'll need to *maintain your own backup* of your @files/@
...
h3. Is a "Spinning" Task Always Still Running?
...
h3. Why Delete the Site Node for a Stuck "Clone" or "Migrate"?
...
But Aegir will _also_ fail to verify this broken site node in the old
